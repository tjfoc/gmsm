...
var (
	...
	oidSHA1   = asn1.ObjectIdentifier{1, 2, 840, 113549, 2, 7}
	oidSHA256 = asn1.ObjectIdentifier{1, 2, 840, 113549, 2, 9}
	oidSHA512 = asn1.ObjectIdentifier{1, 2, 840, 113549, 2, 11}
...
func parsePKCS8EcryptedPrivateKey(der, pwd []byte) (*PrivateKey, error) {
	...
	var key []byte
	switch {
	case pkdf2Params.Prf.Algorithm.Equal(oidMD5):
		key = pbkdf(pwd, salt, iter, 32, md5.New)
		break
	case pkdf2Params.Prf.Algorithm.Equal(oidSHA1):
		key = pbkdf(pwd, salt, iter, 32, sha1.New)
		break
	case pkdf2Params.Prf.Algorithm.Equal(oidSHA256):
		key = pbkdf(pwd, salt, iter, 32, sha256.New)
		break
	case pkdf2Params.Prf.Algorithm.Equal(oidSHA512):
...
func marshalSm2EcryptedPrivateKey(PrivKey *PrivateKey, pwd []byte) ([]byte, error) {
	...
	algorithmIdentifier.Algorithm = oidSHA1
